-- wanek container utlization UPDATED  ON AUG.29.2021 
...
between char(current date - 155 days) and char(current DATE)  AND substr(a.WCICONTAINERNUMBER,1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1','AAII','ARRR')
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('35')  AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 150 days) and char(current DATE) 
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 150 days) and char(current DATE)  and a.WCHORIGIN in ('35') and 
...
-- ----------------------------------------------------------------------------------------------------------------------------------------
-- Updated on Aug.30.2021 for wanek Container Utilization Summary sheet
