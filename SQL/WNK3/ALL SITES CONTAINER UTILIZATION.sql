-- WN3 Updated on Feb.18.2022 for wanek Container Utilization Summary sheet
...
(CASE 
         ...
         WHEN  CONTAINER# LIKE ('35%') and CONTAINER# LIKE ('%-335%') and TRIM(WCHDOORNUMBER) LIKE ('0%') THEN 'BW'
         WHEN  CONTAINER# LIKE ('35%') and CONTAINER# LIKE ('%-CNW%') and TRIM(WCHDOORNUMBER) LIKE ('0%') THEN 'BW'
         WHEN  CONTAINER# LIKE ('35%') and CONTAINER# LIKE ('%-C') and TRIM(WCHDOORNUMBER) LIKE ('0%') THEN 'BW'
...
AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE) 
...
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('B1','V3','M3')))
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE)  and a.WCHORIGIN in ('35') and 
substr(trim(a.WCHCONTAINERNUMBER),1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1')) as x1



-- WN2 Updated on Feb.18.2022 for wanek Container Utilization Summary sheet
SELECT 
x1.WCHDOORNUMBER,x1.WCHCONTAINERNUMBER,x1.WCHORIGIN,x1.WCHDESTINATION,x1.WCHCONTAINERSTATUS,x1.WCHTOTALCARTONS,x1.WCHTOTALCUBES,x1.WCHPOSTEDTIMESTAMP,x1.WCHTOTALWEIGHT,x1.WCHCONTAINERSIZE,
x1.Container#,to_char(x1.WCHPOSTEDTIMESTAMP,'yyyy-mm-dd') as Date, x1.WCHPOSTEDTIMESTAMP,x1.WCHPOSTEDUSER,
(CASE 
...
trim(a.WCHORIGIN)||'-'|| trim(a.WCHCONTAINERNUMBER)||'-'||trim(a.WCHDESTINATION) as Container#
FROM  WWUSAF.WVCNTHD a
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('33') and a.WCHBUILDING in ('33') and a.WCHCONTAINERNUMBER not like 'SUNR2%'  
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')  
AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE) 
and substr(trim(a.WCHCONTAINERNUMBER),1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1')
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('33') and a.WCHBUILDING in ('33') and a.WCHCONTAINERNUMBER not like 'SUNR2%'  
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')  
AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE)  and 
substr(trim(a.WCHCONTAINERNUMBER),1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1')) as x1


-- WN1 Updated on Feb.18.2022 for wanek Container Utilization Summary sheet
...
trim(a.WCHORIGIN)||'-'|| trim(a.WCHCONTAINERNUMBER)||'-'||trim(a.WCHDESTINATION) as Container#
FROM  WWUSAF.WVCNTHD a
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('31')  and (a.WCHACTUALARRIVALMAINTPROGRAM='SVCHECKIN' 
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('K1'))) and a.WCHCONTAINERNUMBER not like 'SUNR2%'  
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
...
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('K1'))) and a.WCHCONTAINERNUMBER not like 'SUNR2%' 
AND a.WCHCONTAINERNUMBER not like 'AIR%'  AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000'  and char(current DATE)  and a.WCHORIGIN in ('31') and 
substr(trim(a.WCHCONTAINERNUMBER),1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1')) as x1
...
-- MIL Container Utilization Summary report updated on Feb.18.2022 by Jimshen
Select 
x1.WCHDOORNUMBER,x1.WCHCONTAINERNUMBER,x1.WCHORIGIN,x1.WCHDESTINATION,x1.WCHCONTAINERSTATUS,x1.WCHTOTALCARTONS,x1.WCHTOTALCUBES,x1.WCHPOSTEDTIMESTAMP,x1.WCHTOTALWEIGHT,x1.WCHCONTAINERSIZE,
x1.Container#,to_char(x1.WCHPOSTEDTIMESTAMP,'yyyy-mm-dd') as Date, x1.WCHPOSTEDTIMESTAMP, x1.WCHPOSTEDUSER,
(CASE 
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('51')  AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE) 
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHPOSTEDTIMESTAMP BETWEEN '2021-01-01 00:00:00.000000' and char(current DATE)  and a.WCHORIGIN in ('51') and 
