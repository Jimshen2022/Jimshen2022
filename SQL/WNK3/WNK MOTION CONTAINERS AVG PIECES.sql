...
and  a.WCILASTMAINTENANCETIMESTAMP  between char(current date - 95 days) and char(current DATE)  
...
between char(current date - 95 days) and char(current DATE)  AND substr(a.WCICONTAINERNUMBER,1,4) NOT IN ('AAAR','AIIR','AAIR','AIRR','AIR_','AIR1','AAII','ARRR')
...
AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 90 days) and char(current DATE) 
...
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('B1','V3','M3')))
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 90 days) and char(current DATE)  and a.WCHORIGIN in ('35') and 
...
WHERE CONTAINERTYPE IN ('None-Mixed') AND PRODUCT IN ('UPH')