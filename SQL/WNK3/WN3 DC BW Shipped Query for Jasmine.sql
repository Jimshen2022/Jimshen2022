-- Updated on Apr.21.2022 for wanek Container Utilization Summary sheet
...
(CASE 
		 ...
		 WHEN  CONTAINER# LIKE ('35%') and TRIM(WCHDOORNUMBER) LIKE ('5%') THEN 'DC'
...
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('B1','B2','V3','M3')))
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
AND a.WCHPOSTEDTIMESTAMP between char(current date - 180 days) and char(current DATE)
...
OR (a.WCHACTUALARRIVALMAINTPROGRAM not in ('SVCHECKIN') and WCHBUILDING in ('B1','B2','V3','M3')))
AND a.WCHCONTAINERNUMBER not like 'AIR%' AND a.WCHDESTINATION NOT IN ('100','101','102','12','131','01','3','990')
AND a.WCHPOSTEDTIMESTAMP between char(current date - 180 days) and char(current DATE)  and a.WCHORIGIN in ('35') and 
