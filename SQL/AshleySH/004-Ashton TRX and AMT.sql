SELECT X.ITNBR, X.HOUSE, X.ITCLS,X.ORDNO,X.TRQTY, X.DATE,Y.PAMNT "UP($USD)",
(CASE 
...
FROM
(
SELECT T1.TCODE,T1.ORDNO,T1.ITNBR, T1.HOUSE, T2.ITCLS, T1.TRQTY, Date('20'||Substr(T1.UPDDT, 2, 2) || '-'||  Substr(T1.UPDDT, 4, 2)|| '-' ||substr(T1.UPDDT, 6, 2)) AS "DATE"
...
AND T1.UPDDT BETWEEN '1190101' AND '1291231'
AND T1.TRQTY<>0 AND T1.TCODE in ('RP') 
ORDER BY T1.ITNBR 
) X
