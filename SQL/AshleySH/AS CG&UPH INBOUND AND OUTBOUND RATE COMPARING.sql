...
SELECT s1.type,s1.Product, s1.Year, s1.Week, SUM(s1.QTY) AS "Q'TY" 
...
WHERE t2.ITNBR = t1.ITNBR AND t2.STID = t3.STID AND t1.HOUSE = t3.WHID AND ((t1.HOUSE IN ('232','21','22','PC1')) 
AND (t1.UPDDT>=? And t1.UPDDT<=?) AND (t1.TRQTY<>0) and (t1.TCODE='SA' OR t1.TCODE='RP' OR t1.TCODE='RC'))
) AS s1

where s1.type in ('IN')
GROUP BY s1.type,s1.Product, s1.Year, s1.Week




-- AS TRX BY PRODUCT AND INBOUND and Outbound

SELECT s1.type, s1.Year, s1.Week, 
...
where s1.type in ('IN')
GROUP BY s1.type, s1.Year, s1.Week
ORDER BY s1.Year, s1.Week
