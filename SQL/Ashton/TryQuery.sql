-- Get Transactions in whse335
SELECT a.DATE,a.ITNBR,
...
FROM (
SELECT z.WEEK,z.DATE,z.ITNBR,z."Type",SUM(z.TRQTY) QTY, SUM(z."AMT($USD)") "AMT($USD)"
FROM (
-- get transactoins in and out  
SELECT X.ITNBR, X.ITCLS, X.TRQTY, Y.PAMNT "UP($USD)",X.WEEK,X.DATE,
(CASE 
...
GROUP BY z.WEEK,z.DATE,z.ITNBR,z."Type"
) a
GROUP BY a.DATE,a.ITNBR
ORDER BY a.DATE,a.ITNBR



--  get current on hand and amt summary in 6s'  
SELECT x.WEEK,x.DATE,x.ITNBR, x."Type", SUM(x.MOHTQ) QTY, SUM(x."AMT($USD)") "AMT($USD)"
FROM (
-- CurrentOnHand
...
a.DATE
FROM 
(SELECT T1.ITNBR,T1.HOUSE,T1.ITCLS,T1.MOHTQ,CURRENT DATE AS DATE   
...
GROUP BY x.WEEK,x.DATE,x.ITNBR,x."Type"