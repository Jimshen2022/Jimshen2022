...
AND CHAR(TRIM(t1.TDTAG#)) NOT IN
		...
		(SELECT CHAR(trim(a.WCSSERIALNUMBER)) AS SN
			...
			WHERE a.WCSCONTAINERNUMBER = b.WCHCONTAINERNUMBER and b.WCHCONTAINERSTATUS IN ('P','T') AND b.WCHPOSTEDTIMESTAMP BETWEEN CHAR(CURRENT DATE - 61 days) AND CHAR(CURRENT DATE + 1 days))

AND CHAR(TRIM(t1.TDTAG#)) NOT IN 
			-- Archived container serial numbers in past 90 days
		(SELECT CHAR(TRIM(a.WCSSERIALNUMBER)) AS SN
			FROM ASHLEYARCL.WVCNTSDA a
			WHERE a.WCSADDEDTIMESTAMP between char(current date - 61 days) and char(current DATE + 1 days)) 
