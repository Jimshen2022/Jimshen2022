-- MIL Container Scanned QUERY by JimShen on Mar.17.2022
SELECT *
FROM 
(
...
COUNT(t1.WCSSERIALNUMBER) AS LOADED_QTY, COUNT(CAST(t1.WCSSERIALNUMBER AS FLOAT))/t2.ITMCQTY AS BoxQty, COUNT(t1.WCSSERIALNUMBER)*t3.B2Z95S AS CUBES
		
FROM LLUSAF.WVCNTSD t1, AFILELIBL.ITMEXT t2, AMFLIBL.ITMRVA t3
WHERE t1.WCSADDEDTIMESTAMP BETWEEN ? AND ? AND t1.WCSITEMNUMBER= t2.ITNBR AND t1.WCSCONTAINERNUMBER NOT LIKE  'KHOA%' AND t1.WCSITEMNUMBER = t3.itnbr AND t1.WCSORIGIN=t3.stid
...
ORDER BY x.WCSCONTAINERNUMBER, x.DATE, x.HOUR