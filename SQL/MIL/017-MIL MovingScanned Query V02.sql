...
SELECT X1.SN, 1 as QTY,X1.AACOD1,X1.AATWHS,X1.Location,Y1.AAORD#,X1.AAITM#,X1.AAEMP#,X1.AAAUSR,
X1.SHIFT, X1.ITMCQTY, X1.ScannedTime,HOUR(X1.ScannedTime) AS HOUR,X1.Cartons,X1.ITCLS,X1.Product
...
substr(char(TIME(to_date(a.AAADAT||' '||right('000000'||ltrim(a.AAATIM),6), 'yyyymmdd hh24:mi:ss'))),7,2)) BETWEEN '070000' AND '194459' THEN 'DS' ELSE 'NS' END) AS SHIFT,t2.ITMCQTY,t3.ITCLS,
(CASE 
...
WHERE a.AAFARA IN ('RM') AND a.AACOD1 IN ('MV') and a.AASER#>0 AND a.AATARA LIKE 'HJ%'
and CHAR(to_date(a.AAADAT||' '||right('000000'||ltrim(a.AAATIM),6),'yyyymmdd hh24:mi:ss')) BETWEEN ?  AND ? AND a.AAITM# = t2.ITNBR  and a.AAITM# = t3.ITNBR
		
) AS X1

LEFT JOIN 
-- To find MO for table X1
