-- MIL Container Utilization Summary report updated on Nov.23 by Jimshen
Select 
...
(CASE 
	WHEN TRIM(SUBSTR(x1.WCHCONTAINERSIZE,1,2)) = '53' THEN x1.WCHTOTALCUBES/3831
	WHEN TRIM(SUBSTR(x1.WCHCONTAINERSIZE,1,2)) = '50' THEN x1.WCHTOTALCUBES/3333
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHORIGIN IN ('51')  AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 360 days) and char(current DATE) 
...
WHERE a.WCHCONTAINERSTATUS in ('P','T') AND a.WCHPOSTEDTIMESTAMP BETWEEN char(current date - 360 days) and char(current DATE)  and a.WCHORIGIN in ('51') and 
