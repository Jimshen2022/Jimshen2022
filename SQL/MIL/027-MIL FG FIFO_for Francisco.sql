-- MIL F/G EOL scanned time to loading time created by Jimshen on Aug.03.2022
...
WHERE  t1.AAADAT BETWEEN  CHAR(VARCHAR_FORMAT(current date -60 days,'YYYYMMDD'))  AND CHAR(VARCHAR_FORMAT(current date, 'YYYYMMDD')) 
 AND t1.AACOD1 IN ('MF') AND t1.AACOD2 IN ('SN') AND TRIM(t1.AAITM#) = t3.ITNBR
...
WHERE  t1.AAADAT BETWEEN  CHAR(VARCHAR_FORMAT(current date -60 days,'YYYYMMDD'))  AND CHAR(VARCHAR_FORMAT(current date, 'YYYYMMDD')) 
 AND t1.AACOD1 IN ('MF') AND t1.AACOD2 IN ('IT')
) a2 ON a1.SN = a2.SN
) a3
...
FROM DISTLIBL.ACTAUDT t1
WHERE  t1.AAADAT BETWEEN  CHAR(VARCHAR_FORMAT(current date -60 days,'YYYYMMDD'))  AND CHAR(VARCHAR_FORMAT(current date, 'YYYYMMDD')) 
 AND t1.AACOD1 IN ('BT') AND t1.AAAPGM IN ('HJ361E')
