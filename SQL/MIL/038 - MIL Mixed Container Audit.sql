...
select a8.WCIORIGIN,a8.CONTAINER#,a8.CONTAINERTYPE,replace(replace(xml2clob(xmlagg(xmlelement(NAME A, a8.PRODUCT||','))),'<A>',''),'</A>','') as PRODUCT,a8.Product_Category_Count, a8.Total_Weight,a8.Total_Cubes
FROM
(
SELECT a9.WCIORIGIN,a9.CONTAINER#,a9.PRODUCT,a9.CONTAINERTYPE,a9.Product_Category_Count,a9.Total_Weight,a9.Total_Cubes
FROM 
...
WCILASTMAINTENANCEUSER,ITMCQTY,UNITCUBE,UNITWEIGHT,CARTONS,CONTAINERTYPE,Date,WCHCONTAINERSIZE,Product_Category_Count,Total_Weight,Total_Cubes,
(case 
when substr(WCHCONTAINERSIZE,1,2) = '53' then CUBES/3831
when substr(WCHCONTAINERSIZE,1,2) = '50' then CUBES/3333
when substr(WCHCONTAINERSIZE,1,3) = '40H' then CUBES/2650
when substr(WCHCONTAINERSIZE,1,3) = '40' then CUBES/2383
...
s2.Container#,s2.Product_Category_Count,s2.Total_Weight,s2.Total_Cubes
FROM 
...
(CASE 
        ...
        WHEN c.ITCLS NOT LIKE 'Z%' THEN 'RawMaterial'
        ELSE 'Check' END) AS Product
FROM ASHLEYARCL.TBL_WVCONTAINER_DTL_ITM_A as a, AFILELIBL.ITMEXT as b, AMFLIBL.ITMRVA as c
...
count(Distinct PRODUCT) as Product_Category_Count,sum(TotalWeight) as Total_Weight,sum(Cubes) as Total_Cubes
FROM 
((SELECT trim(a.WCICONTAINERNUMBER) as ContainerNumber, a.WCIORIGIN, a.WCIDESTINATION, a.WCIORDER, trim(a.WCIITEMNUMBER) as ItemNumber, a.WCIQUANTITYLOADED as Qty, 
a.WCILASTMAINTENANCETIMESTAMP, a.WCILASTMAINTENANCEUSER, b.ITMCQTY, c.itcls,c.B2Z95S as UnitCube, a.WCIQUANTITYLOADED*c.WEGHT as TotalWeight, a.WCIQUANTITYLOADED*c.B2Z95S as Cubes,
CEIL(a.WCIQUANTITYLOADED/b.ITMCQTY) as Cartons,
trim(a.WCIORIGIN)||'-'|| trim(a.WCICONTAINERNUMBER)||'-'||trim(a.WCIDESTINATION) as Container#,
(CASE 
        WHEN c.ITCLS LIKE 'TAF%' THEN 'RP'
...
FROM  ASHLEYARCL.TBL_WVCONTAINER_HDR_A a
...
GROUP BY a9.WCIORIGIN,a9.CONTAINER#,a9.PRODUCT,a9.CONTAINERTYPE,a9.Product_Category_Count,a9.Total_Weight,a9.Total_Cubes
) a8
GROUP BY a8.WCIORIGIN,a8.CONTAINER#,a8.CONTAINERTYPE,a8.Product_Category_Count,a8.Total_Weight,a8.Total_Cubes