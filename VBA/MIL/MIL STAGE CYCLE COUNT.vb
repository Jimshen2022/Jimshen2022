...
PW = Sheet3.Range("a2").Value
    ...
    cmdtxt = "Select Y1.SN,Y1.TDTSTS,Y1.TDITEM,Y1.MO,Y1.TDWHSE,Y1.TDMDAT,Y1.TDMTME,Y1.TXT_TIME,Y1.ITCLS,(1/Y1.ITMCQTY) as Cartons, Y1.Product,Y1.SHIFT,Y1.Line,Y2.CTN,Y2."CTN_Status" " & _
			...
			"AND CHAR(TRIM(t1.TDTAG#)) NOT IN (SELECT CHAR(trim(a.WCSSERIALNUMBER)) AS SN FROM LLUSAF.WVCNTSD a, LLUSAF.WVCNTHD b  " & _
			"WHERE a.WCSCONTAINERNUMBER = b.WCHCONTAINERNUMBER and b.WCHCONTAINERSTATUS IN ('P','T') AND b.WCHPOSTEDTIMESTAMP BETWEEN CHAR(CURRENT DATE - 90 days) AND CHAR(CURRENT DATE + 1 days)) " & _
			"AND CHAR(TRIM(t1.TDTAG#)) NOT IN (SELECT CHAR(TRIM(a.WCSSERIALNUMBER)) AS SN FROM ASHLEYARCL.WVCNTSDA a WHERE a.WCSADDEDTIMESTAMP between char(current date - 90 days) and char(current DATE + 1 days))) AS X1 " & _
			...
			"WHERE a.WCSCONTAINERNUMBER = b.WCHCONTAINERNUMBER and b.WCHCONTAINERSTATUS NOT IN ('P','T')) AS Y2 ON Y1.SN = Y2.SN "
