...
PW = Sheet3.Range("a2").Value
    ...
    cmdtxt = "Select Y1.SN,Y1.TDTSTS,Y1.TDITEM,Y1.MO,Y1.TDWHSE,Y1.TDMDAT,Y1.TDMTME,Y1.TXT_TIME,Y1.ITCLS,(1/Y1.ITMCQTY) as Cartons, Y1.Product,Y1.SHIFT,Y1.Line,Y2.CTN,Y2.""CTN_Status"" " & _
            ...
            "AND NOT EXISTS (SELECT 1 FROM ((SELECT CHAR(trim(a.WCSSERIALNUMBER)) AS SN FROM LLUSAF.WVCNTSD a, LLUSAF.WVCNTHD b WHERE a.WCSCONTAINERNUMBER = b.WCHCONTAINERNUMBER and b.WCHCONTAINERSTATUS IN ('P','T') AND b.WCHPOSTEDTIMESTAMP BETWEEN CHAR(CURRENT DATE - 61 days) AND CHAR(CURRENT DATE + 1 days)) " & _
            "Union ALL (SELECT CHAR(TRIM(a.WCSSERIALNUMBER)) AS SN FROM ASHLEYARCL.WVCNTSDA a WHERE a.WCSADDEDTIMESTAMP between char(current date - 61 days) and char(current DATE + 1 days))) XX1 WHERE CHAR(trim(t1.TDTAG#)) = XX1.SN)) AS X1 " & _
