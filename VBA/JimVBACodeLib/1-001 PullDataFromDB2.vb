...
'make string from excludeLocation list
Dim i%, j%, nrow%, str As String, arr
    nrow = Sheet1.Range("a1048576").End(3).Row
    arr = Sheet1.Range("a1").CurrentRegion
    
    'make exception locations list = str
    For i = 1 To nrow
        If i < nrow Then
            str = str & "'" & arr(i, 1) & "'" & ","
        Else
...
cmdtxt = "SELECT a1.ITNBR, a1.ITDSC, a1.ITCLS, a1.HOUSE, a1.LLOCN, a1.LQNTY, a1.ORDNO, a1.LBHNO, " & _
...
" FROM AMFLIBL.SLQNTY t1 left join AMFLIBL.ITMRVA t2 on t1.itnbr = t2.itnbr WHERE t1.HOUSE IN ('51') and t1.LLOCN NOT IN (" & str & ")) a1 " & _
...
" ORDER BY x.RPAITX, x.RPAMVA ASC) b1  GROUP BY b1.RPAITX ) a2  ON a1.ITNBR = a2.RPAITX "
    ...
    Sheet5.Columns("a:k").AutoFit
    Sheet5.Range("m1").Value = "DataCollectedAt: " & Format(Now(), "yyyy/mm/dd  hh:mm:ss")
    Sheet3.Range("m1").Value = Sheet5.Range("m1").Value
